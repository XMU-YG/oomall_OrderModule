...
public class WangxianweiTest {
    ...
    public void updateShareActivity1() throws Exception {
        String token = adminLogin("13088admin", "123456");
        //新建一个用于测试
        byte[] responseString2 = manageClient.post().uri("/shops/0/goods/502/shareactivities").header("authorization", token)
                .bodyValue("{\n" +
                        "\t\"beginTime\":\"2021-11-30 23:59:00\",\n" +
                        "\t\"endTime\":\"2021-12-15 23:23:23\",\t\n" +
                        "\t\"strategy\":\"{\\\"rule\\\" :[{ \\\"num\\\" :0, \\\"rate\\\":1 },{ \\\"num\\\" :10, \\\"rate\\\":10}],\\\"firstOrAvg\\\" : 0}\"\n" +
    ...
    public void onlineShareActivity1() throws Exception {
        ...
        JSONObject jsonObject = JSONObject.parseObject(new String(responseString2)).getJSONObject("data");
        this.shareActivityId = jsonObject.getLong("id");
        byte[] responseString = manageClient.put().uri("/shops/0/shareactivities/"+this.shareActivityId+"/online").header("authorization", token)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
    ...
    public void deleteShareActivity3() throws Exception {
        ...
        JSONObject jsonObject = JSONObject.parseObject(new String(responseString2)).getJSONObject("data");
        this.shareActivityId = jsonObject.getLong("id");
        manageClient.put().uri("/shops/0/shareactivities/"+this.shareActivityId+"/online").header("authorization", token)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
                .returnResult().getResponseBodyContent();
        byte[] responseString = manageClient.delete().uri("/shops/0/shareactivities/"+this.shareActivityId).header("authorization", token)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
    ...
    public void addShares() throws Exception {
        ...
        //上线一个501商品已有的分享活动
        manageClient.put().uri("/shops/0/shareactivities/304113/online").header("authorization", tokenAdmin)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
    ...
    public void addShares2() throws Exception {
        String token = userLogin("36040122840", "123456");
        byte[] responseString = mallClient.post().uri("/skus/0/shares").header("authorization", token)
                .exchange()
                .expectStatus().isNotFound()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
    ...
    public void getShareActivity2() throws Exception {
        ...
        byte[] responseString = mallClient.get().uri("/shareactivities?shopId=0&skuId=501&page=1&pageSize=1").header("authorization", token)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
